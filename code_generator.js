...
function convert_word(a) {
    if (typeof a === "string") {
        ...
        return multiple_morphemes(a);
}
function generate_atwiki_code(input) {
    ...
    return `#divclass(Fynelipa){&font(150%){${orig_sent.map(str => str.replace(/fh/g, "F")
        .replace(/vh/g, "V")
        .replace(/dz/g, "X")
        .replace(/fъh/g, "fh")
        .replace(/vъh/g, "vh")
        .replace(/dъz/g, "dz")).join(" ")}}}
